source("~/Research_projects/tree_cov_reg/config/config_simu.R")
...
### Save results
methods = c("cov_est", "cov_reg", "oracle_cov_reg")

# Gaussian covariance regression on log-odds
if("cov_reg" %in% methods){
  ...
  for(rank in ranks){
    ...
    print(paste0("Running: rank-",rank," covariance regression with GL prior"))
    ...
    for( i in 1:n ){
      ...
      for( iter in (burnin+1):niter ){
        pred_density[iter-burnin] = Density(result[[1]][[iter]], result[[2]][[iter]], X[i,], Phi_est[i,], result[[3]][[iter]])
      }
      lppd = lppd + log(mean(pred_density))
      pwaic2 = pwaic2 + var(log(pred_density))
    }
    WAIC = -1*lppd + pwaic2
    
    note = paste0("Simulation: rank", rank, " covariance regression with estimated log ratio")
    save(TFP_pmean, Theta_pmean, B_pmean, B0_pmean, Sigma_pmean, 
         mse_Sigma, mse_Theta,
         WAIC,
         date_of_run, session_info, note, 
         file = paste0(result_path, prefix, "_R_",rank,"_cov_reg_simulation_results_", touse))
  }
}

# Gaussian covariance regression on latent variable
if("oracle_cov_reg" %in% methods){
  for(rank in ranks){
    set.seed(202408181)
    Theta_init = 0.5*cov(Phi_est)+0.5*diag(p) # The initialization is changed to handle high-dimensional p
    B_init = array(rnorm(p*d*rank), dim = c(p,d,rank))
    ...
    print(paste0("Running: rank-",rank," covariance regression with GL prior"))
    result = GibbsRCovMeanRegGL(Phi = Phi_true, X = X, R = rank, niter = niter,
                                Theta = Theta_init, B = B_init, B0 = B0_init)
    ...
    for( i in 1:n ){
      pred_density = numeric(niter-burnin)
      for( iter in (burnin+1):niter ){
        pred_density[iter-burnin] = Density(result[[1]][[iter]], result[[2]][[iter]], X[i,], Phi_true[i,], result[[3]][[iter]])
      }
      lppd = lppd + log(mean(pred_density))
      pwaic2 = pwaic2 + var(log(pred_density))
    }
    ...
    note = paste0("Simulation: rank", rank, " covariance regression with latent variables")
    save(TFP_pmean, Theta_pmean, B_pmean, B0_pmean, Sigma_pmean,
         mse_Sigma, mse_Theta, 
         WAIC,
         date_of_run, session_info, note, 
         file = paste0(result_path, prefix, "_oracle_R_",rank,"_cov_reg_simulation_results_", touse))
  }
}

### Gaussian covariance estimation on log-odds
if("cov_est" %in% methods){
  print("Running: covariance estimation with GL prior")
  Theta_init = 0.5*cov(Phi_est)+0.5*diag(p)
  S = cov(Phi_est)*n
  result = GibbsCovGL(S=S, n=n, niter = niter, Theta = Theta_init)
  ...
  for(iter in (burnin+1):niter){
    ...
    # ROC
    graph_est = cov2cor(result[[iter]])[upper.tri(Theta)]
    for(j in 1:1000){
      a = (j-1)/1000
      graph_binary = (abs(graph_est) >= a)%>%as.numeric()
      TPR[iter-burnin,j] = sum(graph_true_vec == 1 & graph_binary == 1)/sum(graph_true_vec == 1)
    ...
    # posterior means
    Theta_pmean = Theta_pmean + result[[iter]]/burnin
    Sigma_est = array(NA, c(p,p,n))
    for(j in 1:n){
      Sigma_est[,,j] = result[[iter]]
    }
    Sigma_pmean = Sigma_pmean + Sigma_est/burnin
    
    # posterior MSEs
    mse_Theta[iter-burnin] = norm(Theta -  result[[iter]])^2
    mse_Sigma[iter-burnin] = sum((Sigma - Sigma_est)^2)/n
  }
  ...
  date_of_run = Sys.time()
  session_info = devtools::session_info()
  note = "Simulation: covariance matrix estimation with the graphical LASSO prior"
  save(TFP_pmean, Theta_pmean, Sigma_pmean, mse_Sigma, mse_Theta, 
       date_of_run, session_info, note, 
       file = paste0(result_path, prefix, "_cov_est_simulation_results_", touse))
}
print("Done!")