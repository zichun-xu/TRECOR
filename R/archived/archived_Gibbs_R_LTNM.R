...
GibbsLTNMRCovRegGL = function(Y, N, X, R, niter, r_GL=1, s_GL=0.01,
                              a_c = 5, b_c = 0.5){
  n = nrow(X)
  d = ncol(X)
  p = ncol(Y)
  ...
  for(r in 1:R){
    B[,,r] = rmatnorm(M = matrix(0, p, d), U = Theta, V = C) 
  }
  B = B2B(B)
  ga = rnorm(n*R)%>%matrix(nrow = n, ncol = R)
  
  Theta_list <- B_list <- Phi_list <- list()
  ...
  for(iter in 1:niter){
    ...
    for(j in 1:p){
      ...
      u = rgig(n = 1, lambda = 1-(n+d*R)/2, psi = lambda, 
               chi = chi)
    ...
    for(j in 1:d){
      ...
      sigma2[j]= 1/rgamma(n=1, shape=(R*p)/2+a_c, rate=a/2+b_c)
    }
    C = diag(sigma2)
    C_inv = diag(1/sigma2)
  ...
  return(list(Theta_list, B_list, Phi_list))
}

GibbsLTNMRCovMeanRegGL = function(Y, N, X, R, niter, r_GL=1, s_GL=0.01, 
  ...
  B = array(0, dim = c(p,d,R))
  B0 = matrix(0,p,d)
  C <- D <- C_inv <- D_inv <- diag(d)
  Theta = 0.5*cov(Phi)+0.5*diag(p) # The initialization is changed to handle high-dimensional p
  lambda= rgamma(1,r_GL,s_GL)
  Tau <- matrix(0, p, p)
  for (l in 2:p){
    for (k in 1:(l-1)){
  ...
  Theta_list <- B_list <- B0_list <- Phi_list <- list()
  ...
  return(list(Theta_list, B_list, B0_list, Phi_list))
}
